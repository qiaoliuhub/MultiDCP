...
class MultiDCPSub(nn.Module):

    """
    submodel of the multidcp
    """
    ...
    def __init__(self, drug_input_dim, drug_emb_dim, conv_size, degree, gene_input_dim, gene_emb_dim, num_gene,
        ...
        super(MultiDCPSub, self).__init__()
    ...
    def init_weights(self):
        print('Initialized multidcp sub\'s weight............')
    ...
    def forward(self, input_drug, input_gene, mask, input_pert_type, input_cell_id, input_pert_idose, epoch = 0, linear_only=False):
        ...
        if self.use_cell_id:
            ...
            if linear_only:
                ...
                if epoch % 100 == 80:
                    print('------followings are multidcp sub before linear embed ----------')
                    print(input_cell_id)

                cell_id_embed = self.cell_id_embed_linear_only(input_cell_id)
                if epoch % 100 == 80:
                    print('------followings are multidcp sub after linear embed ----------')
                    print(cell_id_embed)
                ...
                if epoch % 100 == 80:
                    print('------followings are multidcp sub before transformer ----------')
                    print(cell_id_embed)
                ...
                # cell_id_embed = [batch * 50 * 32(trans_cell_embed_dim)]
                if epoch % 100 == 80:
                    print('------followings are multidcp sub after transformer ----------')
                    print(cell_id_embed)
...
class MultiDCP(nn.Module):
    ...
    def __init__(self, drug_input_dim, drug_emb_dim, conv_size, degree, gene_input_dim, gene_emb_dim, num_gene,
        ...
        self.sub_multidcp = MultiDCPSub(drug_input_dim, drug_emb_dim, conv_size, degree, gene_input_dim, gene_emb_dim, num_gene,
    ...
    def forward(self, input_drug, input_gene, mask, input_pert_type, input_cell_id, input_pert_idose, epoch = 0, linear_only=False):
        ...
        # gene_embed = [num_gene * gene_emb_dim]
        # out = [batch * num_gene * hid_dim]
        return self.sub_multidcp(input_drug, input_gene, mask, input_pert_type, input_cell_id, input_pert_idose, epoch = epoch, linear_only = linear_only)
...
class MultiDCPOriginal(MultiDCP):
    ...
    def __init__(self, drug_input_dim, drug_emb_dim, conv_size, degree, gene_input_dim, gene_emb_dim, num_gene,
                 ...
                 pert_type_emb_dim=None, cell_id_emb_dim=None, pert_idose_emb_dim=None, use_pert_type=False,
                 use_cell_id=False, use_pert_idose=False):
        super(MultiDCPOriginal, self).__init__(drug_input_dim, drug_emb_dim, conv_size, degree, gene_input_dim, gene_emb_dim, num_gene,
                 hid_dim, dropout, loss_type, device, initializer=initializer, pert_type_input_dim=pert_type_input_dim,
                 cell_id_input_dim=cell_id_input_dim, pert_idose_input_dim=pert_idose_input_dim,
                 pert_type_emb_dim=pert_type_emb_dim, cell_id_emb_dim=cell_id_emb_dim, pert_idose_emb_dim=pert_idose_emb_dim, 
                 use_pert_type=use_pert_type, use_cell_id=use_cell_id, use_pert_idose=use_pert_idose)
    ...
    def init_weights(self):
        print('Initialized multidcp original\'s weight............')
        super().init_weights()
        print('used original models, no pretraining')
        #print('load old model')
        #self.sub_multidcp.load_state_dict(torch.load('best_mode_ehill_storage_'))
        #print('frozen the parameters')
        #for param in self.sub_multidcp.parameters():
    ...
    def gradual_unfreezing(self, unfreeze_pattern=[True, True, True, True]):
        ...
        self.sub_multidcp.gradual_unfreezing(unfreeze_pattern[:3])
...
class MultiDCPPretraining(MultiDCP):
    ...
    def __init__(self, drug_input_dim, drug_emb_dim, conv_size, degree, gene_input_dim, gene_emb_dim, num_gene,
                 ...
                 pert_type_emb_dim=None, cell_id_emb_dim=None, pert_idose_emb_dim=None, use_pert_type=False,
                 use_cell_id=False, use_pert_idose=False):
        super(MultiDCPPretraining, self).__init__(drug_input_dim, drug_emb_dim, conv_size, degree, gene_input_dim, gene_emb_dim, num_gene,
                 hid_dim, dropout, loss_type, device, initializer=initializer, pert_type_input_dim=pert_type_input_dim,
                 cell_id_input_dim=cell_id_input_dim, pert_idose_input_dim=pert_idose_input_dim,
                 pert_type_emb_dim=pert_type_emb_dim, cell_id_emb_dim=cell_id_emb_dim, pert_idose_emb_dim=pert_idose_emb_dim, 
                 use_pert_type=use_pert_type, use_cell_id=use_cell_id, use_pert_idose=use_pert_idose)
...
class MultiDCPEhillPretraining(MultiDCP):
    ...
    def __init__(self, drug_input_dim, drug_emb_dim, conv_size, degree, gene_input_dim, gene_emb_dim, num_gene,
                 ...
                 pert_type_emb_dim=None, cell_id_emb_dim=None, pert_idose_emb_dim=None, use_pert_type=False,
                 use_cell_id=False, use_pert_idose=False):
        super(MultiDCPEhillPretraining, self).__init__(drug_input_dim, drug_emb_dim, conv_size, degree, gene_input_dim, gene_emb_dim, num_gene,
                 hid_dim, dropout, loss_type, device, initializer=initializer, pert_type_input_dim=pert_type_input_dim,
                 cell_id_input_dim=cell_id_input_dim, pert_idose_input_dim=pert_idose_input_dim,
                 pert_type_emb_dim=pert_type_emb_dim, cell_id_emb_dim=cell_id_emb_dim, pert_idose_emb_dim=pert_idose_emb_dim, 
                 use_pert_type=use_pert_type, use_cell_id=use_cell_id, use_pert_idose=use_pert_idose)
    ...
    def init_weights(self, pretrained = False):
        ...
        if pretrained:
            ...
            self.sub_multidcp.load_state_dict(torch.load('best_model_ehill_storage_linear_'))
...
class MultiDCP_AE(MultiDCP):
    ...
    def __init__(self, drug_input_dim, drug_emb_dim, conv_size, degree, gene_input_dim, gene_emb_dim, num_gene,
                 ...
                 use_cell_id=False, use_pert_idose=False):
        super(MultiDCP_AE, self).__init__(drug_input_dim, drug_emb_dim, conv_size, degree, gene_input_dim, gene_emb_dim, num_gene,
                 hid_dim, dropout, loss_type, device, initializer=initializer, pert_type_input_dim=pert_type_input_dim,
                 cell_id_input_dim=cell_id_input_dim, pert_idose_input_dim=pert_idose_input_dim,
                 pert_type_emb_dim=pert_type_emb_dim, cell_id_emb_dim=cell_id_emb_dim, pert_idose_emb_dim=pert_idose_emb_dim, 
                 use_pert_type=use_pert_type, use_cell_id=use_cell_id, use_pert_idose=use_pert_idose)
        ...
        self.relu = nn.ReLU()
        self.trans_cell_embed_dim = self.sub_multidcp.trans_cell_embed_dim
        self.linear_2 = nn.Linear(hid_dim, 1)
    ...
    def forward(self, input_drug, input_gene, mask, input_pert_type, input_cell_id, input_pert_idose,
        ...
        if job_id == 'perturbed':
            ...
            if linear_only:
                # input_cell_id = [batch * 978]
                cell_hidden_ = self.sub_multidcp.cell_id_embed_linear_only(hidden)
                # cell_hidden_ = [batch * cell_id_emb_dim(32)]
                out_2 = self.decoder_linear(cell_hidden_)
            else:
                hidden = self.sub_multidcp.cell_id_embed(hidden)
                if epoch % 100 == 80:
                ...
                # hidden = self.sub_multidcp.cell_id_embed_1(hidden) # Transformer
                # hidden = [batch * cell_id_emb_dim * trans_cell_embed_dim(32))]
                hidden = self.sub_multidcp.pos_encoder(hidden)
                hidden = self.sub_multidcp.cell_id_transformer(hidden, hidden)
        ...
        for name, parameter in self.named_parameters():
            if 'drug_gene_attn' not in name:
                if parameter.dim() == 1:
                    ...
                    self.initializer(parameter)
        if pretrained:
            self.sub_multidcp.load_state_dict(torch.load('best_model_ehill_storage_'))
            # if 'attn' not in name:
            #     if parameter.dim() == 1:
            #         nn.init.constant_(parameter, 10**-7)
            #     else:
            #         self.initializer(parameter)
        #print('load old model')
        #self.sub_multidcp.load_state_dict(torch.load('best_mode_storage_'))
        #print('frozen the parameters')
        #for param in self.sub_multidcp.parameters():
        #    param.requires_grad = False
