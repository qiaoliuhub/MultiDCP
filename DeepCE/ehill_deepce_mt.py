...
for epoch in range(max_epoch):
    ...
    model.train()
    epoch_loss_ehill = 0
    for i, (ft, lb, cell_type) in enumerate(
        ...
        if hill_data.use_pert_idose:
            ...
            pert_idose = None
        optimizer.zero_grad()
        predict, cell_hidden_ = model(drug, hill_data.gene, mask, pert_type, cell_id, pert_idose, epoch=epoch)
    ...
    with torch.no_grad():
        for i, (ft, lb, _) in enumerate(hill_data.get_batch_data(dataset='dev', batch_size=batch_size, shuffle=False)):
            ...
            if hill_data.use_pert_idose:
                ...
                pert_idose = None
            predict, _ = model(drug, hill_data.gene, mask, pert_type, cell_id, pert_idose, epoch=epoch)
...
best_dev_epoch + 1, pearson_ehill_list_dev[best_dev_epoch]))
print("Epoch %d got Spearman's correlation of ehill on dev set: %.4f" % (
best_dev_epoch + 1, spearman_ehill_list_dev[best_dev_epoch]))
